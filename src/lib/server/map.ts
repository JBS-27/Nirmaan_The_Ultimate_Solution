import { asDate, asIso, num } from "@/lib/utils";
import type {
  Attendance,
  Bill,
  Material,
...
export function mapMaterial(r: Record<string, unknown>): Material {
  return {
    id: num(r.id),
    projectId: num(r.project_id),
...
export function mapBill(r: Record<string, unknown>): Bill {
  return {
    id: num(r.id),
    projectId: num(r.project_id),
...
export function mapAttendance(r: Record<string, unknown>): Attendance {
  return {
    id: num(r.id),
...
export function mapPhoto(r: Record<string, unknown>): Photo {
  return {
    id: num(r.id),
    projectId: num(r.project_id),
    phaseId: r.phase_id == null ? null : num(r.phase_id),
...
export function mapPro(r: Record<string, unknown>): Professional {
  return {
    id: num(r.id),
