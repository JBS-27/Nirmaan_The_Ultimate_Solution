import { l as findCity } from "./constants-ReRsv3ys.mjs";
import { r as createServerFn } from "./ssr.mjs";
import { c as todayISO, s as num, t as addDays } from "./utils-Dhq_xXK2.mjs";
...
import { a as mapPayment, c as mapPhoto, d as mapProject, i as mapMessage, n as mapBill, o as mapPayout, p as mapWorker, r as mapMaterial, s as mapPhase, t as mapAttendance } from "./map-TuuXPdmi.mjs";
import { t as ensureCatalog } from "./seed-OVWykS8x.mjs";
import { n as estimateRooms, t as estimateProject } from "./estimator-DvUi7E1M.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/projects-Bb9mwrF7.js
async function insertEstimate(sql, projectId, type, plotSqft, floors, city, startDate, targetDate, budget) {
	...
	for (const m of est.materials) await sql`
      insert into materials (
        project_id, phase_id, name, category, unit, qty_needed, qty_ordered, qty_received, qty_used, unit_price, status
      ) values (
        ${projectId}, ${phaseIds[m.phaseKey] ?? null}, ${m.name}, ${m.category}, ${m.unit}, ${m.qtyNeeded},
...
var listProjects = createServerFn({ method: "GET" }).middleware([authMiddleware]).handler(listProjects_createServerFn_handler, async ({ context }) => {
	...
	const rows = profile?.role === "admin" ? await sql`select * from projects order by created_at desc` : await sql`
            select distinct p.*
            from projects p
	...
	for (const r of rows) {
      ...
      `;
		...
		const progress = Math.round(phases.reduce((s, x) => s + num(x.progress) * num(x.estimated_cost), 0) / w);
...
var getProject = createServerFn({ method: "GET" }).middleware([authMiddleware]).validator((id) => id).handler(getProject_createServerFn_handler, async ({ context, data: id }) => {
	const sql = await getSql();
	...
	const risks = [];
	if (spent > project.budget) risks.push({
		level: "danger",
		title: "Budget exceeded",
		detail: `Spend is over the ${project.name} envelope. Freeze extras until you re-baseline.`
	});
	else if (spent > project.budget * .85) risks.push({
		level: "warn",
		title: "Budget tightening",
		detail: "Over 85% of the envelope is committed. Watch finishing allowances."
	});
	for (const ph of phases) if (ph.endDate < today && ph.progress < 100 && ph.status !== "done") risks.push({
		level: "warn",
		title: `${ph.name} is behind`,
		detail: `Planned finish ${ph.endDate}, progress ${ph.progress}%.`
	});
	if (crewPending > 2e4) risks.push({
		level: "info",
		title: "Crew payouts pending",
		detail: "Outstanding wages will hit cash this week if you close attendance."
	});
	const lowMat = materials.filter((m) => m.qtyReceived + .01 < m.qtyNeeded * .3 && m.status === "needed");
	if (lowMat.length) risks.push({
		level: "info",
		title: "Materials not ordered",
		detail: `${lowMat.length} items still at zero receipt — including ${lowMat[0].name}.`
	});
	return {
		project,
		phases,
...
var createProject = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((input) => input).handler(createProject_createServerFn_handler, async ({ context, data }) => {
	...
	const budget = Math.max(1e5, Math.round(data.budget));
	const id = (await sql`
      ...
      returning id
    `)[0].id;
	const est = await insertEstimate(sql, id, data.projectType, plot, floors, city.name, data.startDate, data.targetDate, budget);
	...
	if (data.sample) {
		const [ph0] = await sql`select id from phases where project_id = ${id} order by sort_order limit 1`;
      ...
      `;
		const wrows = await sql`select id from workers where project_id = ${id}`;
		for (const w of wrows) await sql`
	...
	return {
		...
		builtUpSqft: est.builtUpSqft,
		notes: est.notes
	};
});
var updatePhase_createServerFn_handler = createServerRpc({
	id: "49efee787eaf1e360b9a6af39e6b37e056a5abe3b6643ae7a1f2a44ac9d1ba8a",
	name: "updatePhase",
	filename: "src/lib/server/projects.ts"
}, (opts) => updatePhase.__executeServer(opts));
var updatePhase = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((input) => input).handler(updatePhase_createServerFn_handler, async ({ context, data }) => {
	const sql = await getSql();
	...
	await sql`
      update phases set progress = ${progress}, status = ${data.status ?? (progress >= 100 ? "done" : progress > 0 ? "active" : "upcoming")}
      where id = ${data.phaseId} and project_id = ${data.projectId}
    `;
	return { ok: true };
});
var previewEstimate_createServerFn_handler = createServerRpc({
	id: "09d39eec2717a8ac11b7083e4dbd7fc0c52650154f45b772ed30f4b59e45f785",
	name: "previewEstimate",
	filename: "src/lib/server/projects.ts"
}, (opts) => previewEstimate.__executeServer(opts));
var previewEstimate = createServerFn({ method: "POST" }).validator((input) => input).handler(previewEstimate_createServerFn_handler, async ({ data }) => estimateProject(data));
//#endregion
export { createProject_createServerFn_handler, getProject_createServerFn_handler, listProjects_createServerFn_handler, previewEstimate_createServerFn_handler, updatePhase_createServerFn_handler };
